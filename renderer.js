...
function loadInvoiceForEdit(invoice) {
    ...
    items.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <td>${formatNumber(item.quantity)} Pc(s)</td>
    ...
    addButton.addEventListener('click', () => {
        try {
            ...
            row.innerHTML = `
                ...
                <td>${formatNumber(item.quantity)} Hrs</td>
                <td>${formatNumber(item.unitPrice)}</td>
                <td>${formatNumber(item.subtotal)}</td>
            `;
...
Total: ${formatNumber(invoice.total)}`;
        ...
        try {
            ...
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(10);
            doc.text('Name', startX + 2, startY + 6, { maxWidth: colWidths[0] - 4 });
            doc.text('Designation', startX + colWidths[0] + 2, startY + 6, { maxWidth: colWidths[1] - 4 });
            doc.text('Total Hours', startX + colWidths[0] + colWidths[1] + 2, startY + 6, { maxWidth: colWidths[2] - 4 });
            doc.text('Price/Hour', startX + colWidths[0] + colWidths[1] + colWidths[2] + 2, startY + 6, { maxWidth: colWidths[3] - 4 });
            doc.text('Subtotal', startX + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + 2, startY + 6, { maxWidth: colWidths[4] - 4 });
            ...
            items.forEach((item, index) => {
                ...
                if (currentY + rowHeight > maxContentHeight) {
                    ...
                    doc.text('Name', startX + 2, currentY + 6, { maxWidth: colWidths[0] - 4 });
                    doc.text('Designation', startX + colWidths[0] + 2, currentY + 6, { maxWidth: colWidths[1] - 4 });
                    doc.text('Total Hours', startX + colWidths[0] + colWidths[1] + 2, currentY + 6, { maxWidth: colWidths[2] - 4 });
                    doc.text('Price/Hour', startX + colWidths[0] + colWidths[1] + colWidths[2] + 2, currentY + 6, { maxWidth: colWidths[3] - 4 });
                    doc.text('Subtotal', startX + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + 2, currentY + 6, { maxWidth: colWidths[4] - 4 });
                ...
                const totalHours = `${formatNumber(item.quantity)} Hrs`;
